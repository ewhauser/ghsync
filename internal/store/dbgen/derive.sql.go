...
const applyDerivedWorkItemBatch = `-- name: ApplyDerivedWorkItemBatch :many
...
SELECT $1, kind, identity_key, clock_timestamp(),
       jsonb_build_object(
           'version', 1,
...
func (q *Queries) ApplyDerivedWorkItemBatch(ctx context.Context, arg ApplyDerivedWorkItemBatchParams) ([]int64, error) {
	rows, err := q.db.Query(ctx, applyDerivedWorkItemBatch,
		arg.Stream,
		arg.ScopeKeys,
		arg.Items,
		arg.ChangedKind,
...
const claimDerivationDirtyScopes = `-- name: ClaimDerivationDirtyScopes :many
SELECT scope_key
...
func (q *Queries) ClaimDerivationDirtyScopes(ctx context.Context, dirtyCap int32) ([]string, error) {
	rows, err := q.db.Query(ctx, claimDerivationDirtyScopes, dirtyCap)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&scope_key); err != nil {
			return nil, err
		}
		items = append(items, scope_key)
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
...
const clearDerivationDirtyScopes = `-- name: ClearDerivationDirtyScopes :exec
...
func (q *Queries) ClearDerivationDirtyScopes(ctx context.Context, scopeKeys []string) error {
	_, err := q.db.Exec(ctx, clearDerivationDirtyScopes, scopeKeys)
	return err
}

const loadDerivationSnapshot = `-- name: LoadDerivationSnapshot :many
WITH requested AS (
    SELECT element->>'scope_key' AS scope_key,
...
requested_repos AS (
    SELECT requested.scope_key, requested.kind, requested.installation_id, requested.repo_id, requested.number,
...
selected_prs AS (
    SELECT requested_repos.scope_key, pull_requests.id, pull_requests.repo_id, pull_requests.gh_id, pull_requests.node_id, pull_requests.number, pull_requests.title, pull_requests.state, pull_requests.draft, pull_requests.author_login, pull_requests.head_ref, pull_requests.head_sha, pull_requests.base_ref, pull_requests.base_sha, pull_requests.review_decision, pull_requests.mergeable_state, pull_requests.stack_number, pull_requests.stack_position, pull_requests.gh_updated_at, pull_requests.synced_at, pull_requests.etag, pull_requests.sync_source, pull_requests.tombstoned_at, pull_requests.last_checked_at, pull_requests.display_until
...
func (q *Queries) LoadDerivationSnapshot(ctx context.Context, scopes []byte) ([]LoadDerivationSnapshotRow, error) {
	rows, err := q.db.Query(ctx, loadDerivationSnapshot, scopes)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []LoadDerivationSnapshotRow
	for rows.Next() {
		var i LoadDerivationSnapshotRow
		if err := rows.Scan(
			&i.ScopeKey,
			&i.OrgID,
			&i.RepoID,
