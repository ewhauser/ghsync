-- Q11/Q12: drift sampling ranges need the authoritative validation timestamp
-- so the detector can fetch without a lock, then reject a sample whose cache
-- observation moved before the lock was acquired.
